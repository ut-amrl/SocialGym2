...
class EnteringZone(Observation):
    """
    Tracks if the agent is entering the zone (entering is defined as the agent overlapping with the left most
    ...
    def __observations__(self, env: RosSocialEnv, env_response: ManualZoneUTMRSResponse) -> np.array:
        return np.array([1. if env_response.entering else 0.])
